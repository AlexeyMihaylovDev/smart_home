...
const ClockWidget = () => {
  ...
  return (
    <div className="h-full p-4 sm:p-6 md:p-8 flex flex-col items-center justify-center bg-gradient-to-br from-dark-bg via-dark-card to-dark-bg rounded-lg border border-dark-border shadow-2xl relative overflow-hidden">
      {/* Декоративные элементы фона - более видимые */}
      ...
      <div className="relative z-10 w-full flex flex-col items-center justify-center">
        {/* Время - крупным шрифтом, более ярким */}
        <div className="text-6xl sm:text-7xl md:text-8xl lg:text-9xl font-light text-white mb-2 sm:mb-4 tracking-tight drop-shadow-lg">
          {timeString}
        </div>

        {/* Дата - под временем, более яркой */}
        {dateString && (
          <div className="text-base sm:text-lg md:text-xl lg:text-2xl font-light text-white mt-2 sm:mt-4 drop-shadow-md">
            {dateString}
          </div>
        )}
        ...
        {clockConfig.name && clockConfig.name !== 'שעון' && (
          <div className="mt-4 sm:mt-6 text-xs sm:text-sm text-white/70 font-medium">
            {clockConfig.name}
          </div>
        )}
      </div>
      ...
      {clockConfig.showSeconds && (
        <div className="absolute bottom-2 right-2 w-2 h-2 bg-green-400 rounded-full animate-pulse shadow-lg"></div>
      )}
    </div>
  )
}
